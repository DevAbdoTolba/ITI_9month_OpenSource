...
echo "1. TEST VALIDATION: Signup Fail (Short Password)"
curl -s -X POST http://localhost:3000/auth/signup -H "Content-Type: application/json" -d '{"username": "user1", "password": "123", "email": "test@test.com"}'
echo -e "\n"

echo "2. TEST VALIDATION: Signup Fail (Invalid Email)"
curl -s -X POST http://localhost:3000/auth/signup -H "Content-Type: application/json" -d '{"username": "user1", "password": "123456", "email": "notanemail"}'
echo -e "\n"

echo "3. SIGN UP User 1 (Triggers Email)"
if ! curl -s -X POST http://localhost:3000/auth/signup -H "Content-Type: application/json" -d '{"username": "user1233212311", "password": "123456", "email": "100978dffc@webxio.pro"}'; then
    echo "ERROR in SIGN UP User 1"
fi
echo -e "\n"

echo "4. SIGN IN User 1 (Triggers Email)"
...
echo "5. SIGN UP & IN User 2"
if ! curl -s -X POST http://localhost:3000/auth/signup -H "Content-Type: application/json" -d '{"username": "user2", "password": "password", "email": "user2@test.com"}' > /dev/null; then
...
echo "6. TEST VALIDATION: Create Post Fail (Empty Title)"
curl -s -X POST http://localhost:3000/posts -H "token: $TOKEN1" -H "Content-Type: application/json" -d '{"title": "", "content": "Valid content"}'
echo -e "\n"

echo "7. User 1 Creates Post"
...
echo "10. User 2 Tries to Update User 1 Post"
...
echo "11. TEST VALIDATION: Comment Fail (No PostId)"
curl -s -X POST http://localhost:3000/comments -H "token: $TOKEN1" -H "Content-Type: application/json" -d '{"content": "Where do I belong?"}'
echo -e "\n"

echo "12. User 1 Creates Comment on their Post"
...
echo "13. User 2 Tries to Delete User 1 Comment"
echo $COMMENT_ID
...
echo "14. User 1 Deletes Own Post"
