...
POST_ID=$(curl -s -X POST http://localhost:3000/posts -H "Authorization: Bearer $TOKEN1" -H "Content-Type: application/json" -d '{"title": "User1 Secret Diary", "content": "Nobody should see this"}' | grep -o '"_id":"[^"]*' | grep -o '[^"]*$')
if [ -z "$POST_ID" ]; then
...
if ! curl -s -X GET http://localhost:3000/posts -H "Authorization: Bearer $TOKEN1" | grep "User1 Secret Diary"; then
...
if ! curl -s -X GET http://localhost:3000/posts -H "Authorization: Bearer $TOKEN2"; then
...
if ! curl -s -X PUT http://localhost:3000/posts/$POST_ID -H "Authorization: Bearer $TOKEN2" -H "Content-Type: application/json" -d '{"title": "HACKED"}'; then
    echo "ERROR in User 2 Tries to Update User 1 Post"
fi
echo -e "\n"

echo "8. User 1 Creates Comment on their Post"
COMMENT_ID=$(curl -s -X POST http://localhost:3000/comments -H "Authorization: Bearer $TOKEN1" -H "Content-Type: application/json" -d '{"content": "My private comment", "postId": "$POST_ID"}' | grep -o '"_id":"[^"]*' | grep -o '[^"]*$')
...
if ! curl -s -X DELETE http://localhost:3000/comments/$COMMENT_ID -H "Authorization: Bearer $TOKEN2"; then
    echo "ERROR in User 2 Tries to Delete User 1 Comment"
fi
echo -e "\n"

echo "10. User 1 Deletes Own Post"
if ! curl -s -X DELETE http://localhost:3000/posts/$POST_ID -H "Authorization: Bearer $TOKEN1"; then
